...
static std::vector<network::IndexResult> expected_result;
static std::forward_list<network::IndexParameters_t> index_request;

void define_expected(const data::FileMsg<Data_t::TIME_SERIES,Data_f::GRIB_v1>& msg){
    auto& parameters_struct = std::get<network::IndexParameters<
                Data_t::TIME_SERIES,
                Data_f::GRIB_v1>>(index_request.front());
    if(!parameters_struct.common_data_properties().empty() &&
            parameters_struct.common_data_properties().find(
            Grib1CommonDataProperties(msg.center,msg.table_version,msg.parameter))==
            parameters_struct.common_data_properties().end());
        return;
    if(parameters_struct.grid_type_.has_value() &&
            parameters_struct.grid_type_!=msg.grid_data->type())
        return;
    if(parameters_struct.top_.has_value()||
        parameters_struct.bottom_.has_value()||
        parameters_struct.left_.has_value()||
        parameters_struct.right_.has_value())
    {
        if(parameters_struct.top_.has_value()){
            if(!msg.grid_data->top().has_value())
                return;
            if(msg.grid_data->bottom().has_value()){
                if(parameters_struct.top_.value()<
                    msg.grid_data->bottom().value())
                    return;
            }
            else{
                if(msg.grid_data->top().value()!=
                    parameters_struct.top().value())
                    return;
            }
        }
        if(parameters_struct.bottom_.has_value()){
            if(!msg.grid_data->bottom().has_value())
                return;
            if(msg.grid_data->top().has_value()){
                if(parameters_struct.bottom_.value()>
                    msg.grid_data->top().value())
                    return;
            }
            else{
                if(msg.grid_data->bottom().value()!=
                    parameters_struct.bottom().value())
                    return;
            }
        }
        if(parameters_struct.left_.has_value()){
            if(!msg.grid_data->left().has_value())
                return;
            if(msg.grid_data->right().has_value()){
                if(parameters_struct.left_.value()<
                    msg.grid_data->right().value())
                    return;
            }
            else{
                if(msg.grid_data->left().value()!=
                    parameters_struct.left().value())
                    return;
            }
        }
        if(parameters_struct.top_.has_value()){
...
class DataTestClass:public testing::Test{
    protected:
    std::string fn;
    ...
    void init_request(){
        auto& parameters_struct = std::get<network::IndexParameters<
                    Data_t::TIME_SERIES,
                    Data_f::GRIB_v1>>(
                index_request.emplace_front(
                network::IndexParameters<Data_t::TIME_SERIES,Data_f::GRIB_v1>()));
        parameters_struct.common_.insert(Grib1CommonDataProperties(Organization::ECMWF,128,16));
        parameters_struct.common_.insert(Grib1CommonDataProperties(Organization::ECMWF,228,32));
        parameters_struct.forecast_preference(TimeForecast(TimeFrame::HOUR,
                            TimeRangeIndicator::INIT_REF_TIME,{0},{0}),
                            TimeForecast::LESS_EQ);
...
TEST_F(DataTestClass,Index_DataExchangeTest){
    ...
    additional.add_indexes(std::move(index_request));
    network::ConnectionHandle hconn = Mashroom::instance().connect(
        err,
        "127.0.0.1",
        32396,
        app().config().client_config().current_settings());
    EXPECT_TRUE(hconn.is_valid_handler());
    ASSERT_TRUE(hconn.add_process(
        std::make_unique<network::ClientConnectionProcess>(hconn,err),
        err));
    ASSERT_FALSE(err);
    ...
    if(result->error().has_value() && *result->error())
        ...
        if(result->received() && 
            ...
            std::cout<<"Received INDEX from server. Message: "<<std::setprecision(3)<<std::fixed<<
    ...
    auto add_data = [&](auto& block){
        ...
        if constexpr(std::is_same_v<decay,std::monostate>){
            ...
            // d.add_data(Location<false>::host(path.path_,path.add_.get<path::TYPE::HOST>().port_,utc_tp::clock::now()),block);
