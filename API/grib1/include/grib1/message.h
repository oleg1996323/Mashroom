...
namespace api{
    ...
    struct Message<API_T::GRIB1>{
        ...
        osterlib::ContextedError err_;

        Message(unsigned char* buffer);
    ...
    {   
        private:
        std::unique_ptr<api::Message<API_T::GRIB1>> msg_ = nullptr;
        ...
        std::optional<std::reference_wrapper<
                api::Message<API_T::GRIB1>>> message() const;
        uint64_t current_message_position() const noexcept;
        ...
        osterlib::ContextedError open_grib(const fs::path& filename);
        bool set_message(uint64_t pos) noexcept;
    };
