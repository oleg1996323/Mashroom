...
Array_2D bilinear_interpolation(Array_2D data, int x_sz, int y_sz){
    ...
    if(x_sz == data.nx && y_sz==data.ny){
        ...
        size_t total_assert;
        if(!(result.data = (float*)malloc(sizeof(float)*(x_sz*y_sz)))){
            ...
            for(size_t i = 0;i<result.ny;++i){
                ...
                for(size_t j = 0;j<result.nx;++j){
                    ...
                    if(j>result.nx-result.nx/data.nx && i>result.ny-result.ny/data.ny){
                        result.data[j+result.nx*i] = data.data[data.nx*(i*data.ny/result.ny)+(j*data.nx/result.nx)];
                        --total_assert;
                        if(result.data[j+result.nx*i]<200)
                            fprintf(log,"Pos: %lu, i: %lu, j: %lu value: %.3f\n",j+result.nx*i, i, j,result.data[j+result.nx*i]);
                        continue;
                    }
                    if(j>result.nx-result.nx/data.nx){
                        result.data[j+result.nx*i] = lin_interp_between(data.data[data.nx*(i*data.ny/result.ny)+(j*data.nx/result.nx)],
                                                                        y1,
                                                                        data.data[data.nx*(i*data.ny/result.ny+1)+(j*data.nx/result.nx)],
                                                                        y2,
                                                                        y);
                        --total_assert;
                        if(result.data[j+result.nx*i]<200)
                    ...
                    if(i>result.ny-result.ny/data.ny){
                        result.data[j+result.nx*i] = lin_interp_between(data.data[data.nx*(i*data.ny/result.ny)+(j*data.nx/result.nx)],
                                                                        x1,
                                                                        data.data[data.nx*(i*data.ny/result.ny)+(j*data.nx/result.nx+1)],
                                                                        x2,
                                                                        x);
                        --total_assert;
                        if(result.data[j+result.nx*i]<200)
                    ...
                    float Q12 = data.data[data.nx*((i+1)*data.ny/result.ny)+(j*data.nx/result.nx)];
                    float Q21 = data.data[data.nx*(i*data.ny/result.ny)+((j+1)*data.nx/result.nx)];
                    float Q22 = data.data[data.nx*((i+1)*data.ny/result.ny)+((j+1)*data.nx/result.nx)];
