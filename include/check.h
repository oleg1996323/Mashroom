...
bool missing_files(fs::path in_dir,fs::path out_dir, unsigned int cpus=std::thread::hardware_concurrency(), std::chrono::year_month_day from=1d/1/1991, std::chrono::year_month_day to = std::chrono::floor<std::chrono::days>(std::chrono::system_clock::now())){
    bool result = false;
    std::set<std::chrono::sys_days> found;
    ...
    std::ofstream missing_log(out_dir/"missing_files.txt",std::ios::out);
    if(!missing_log.is_open()){
        ErrorPrint::print_error(ErrorCode::CANNOT_OPEN_FILE_X1,"",AT_ERROR_ACTION::ABORT,(out_dir/"missing_files.txt").c_str());
        ...
        if(entries.size()/cpus>1){
            ...
            for(int i = 0;i<cpus;++i){
                ...
                if(!res.err_files.empty()){
                    std::ofstream corr_files(out_dir/"corrupted_files.txt",std::ios::trunc);
                    if(corr_files.is_open())
                        for(ErrorFiles& err_f:res.err_files){
                            corr_files<<err_f.name<<". Error code: "<<err_f.code<<std::endl;
                        }
                    else
                        ErrorPrint::print_error(ErrorCode::CANNOT_OPEN_FILE_X1,"",AT_ERROR_ACTION::ABORT,(out_dir/"corrupted_files.txt").c_str());
                }
            ...
            if(!res.err_files.empty()){
                std::ofstream err_log(out_dir/"corrupted_files.txt",std::ios::trunc);
                if(err_log.is_open())
                    for(ErrorFiles& err_f:res.err_files){
