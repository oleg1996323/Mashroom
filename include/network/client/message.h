#pragma once
#include <optional>
#include <network/common/def.h>
#include <extract.h>
#include <network/common/message_handler.h>

using namespace std::chrono;
namespace fs = std::filesystem;
namespace client{
enum class TYPE_MESSAGE : uint8_t{
    METEO_REQUEST,
    SERVER_CHECK,
    CHECK_DATA,
};
template<TYPE_MESSAGE>
struct Message;

template<>
struct Message<TYPE_MESSAGE::METEO_REQUEST>{
    static constexpr TYPE_MESSAGE type_msg_ = TYPE_MESSAGE::METEO_REQUEST;
    //further add field that modify type of meteo (grib/netcdf/rp5 etc.)
    std::optional<TimeFrame> t_fcst_;
    std::optional<uint16_t> parameter_sz_;
    std::optional<Organization> center;
    std::optional<utc_tp> from;   //requested from Date
    std::optional<utc_tp> to;     //requested to Date
    std::optional<Coord> pos;   //requested position (in WGS)
    std::optional<RepresentationType> rep_t;
    std::optional<TimeOffset> time_off_;
    std::optional<Extract::ExtractFormat> fmt_ = Extract::ExtractFormat::TXT_F|Extract::ExtractFormat::ARCHIVED;
    Extract prepare_and_check_integrity_extractor(ErrorCode& err) const;
};
struct MessageFileInfo{
    char filename[256];
    uintmax_t data_sz = 0;      //size of file
};
template<>
struct Message<TYPE_MESSAGE::SERVER_CHECK>{
    static constexpr TYPE_MESSAGE type_msg_ = TYPE_MESSAGE::SERVER_CHECK;
    bool sendto(int sock);
};
template<>
struct Message<TYPE_MESSAGE::CHECK_DATA>{
    static constexpr TYPE_MESSAGE type_msg_ = TYPE_MESSAGE::CHECK_DATA;
    utc_tp from = {};
    utc_tp to = {};
    Coord lefttop = {};
    Coord rightbottom = {};
};
constexpr std::array<size_t,3> sizes_msg_struct={
    sizeof(Message<TYPE_MESSAGE::METEO_REQUEST>),
    sizeof(Message<TYPE_MESSAGE::SERVER_CHECK>),
    sizeof(Message<TYPE_MESSAGE::CHECK_DATA>)
};
constexpr size_t num_msg = sizes_msg_struct.size();
using request_message = std::variant<std::monostate,
                        client::Message<TYPE_MESSAGE::METEO_REQUEST>,
                        client::Message<TYPE_MESSAGE::SERVER_CHECK>,
                        client::Message<TYPE_MESSAGE::CHECK_DATA>>;

class MessageProcess:public ::network::MessageHandler<TYPE_MESSAGE,request_message>{
    public:
    MessageProcess(){
        ::network::check_variant_enum<TYPE_MESSAGE,request_message,num_msg>();
    }
};
}