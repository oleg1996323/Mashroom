...
namespace network{
    ...
    class ConnectionPool{
        ...
        std::unordered_set<Socket> peers_;
        std::mutex m;
        public:
        bool contains_socket(int raw_socket) const{
            return peers_.contains(raw_socket);
        }
        const Socket& get_socket(int raw_socket) const{
    ...
    template<typename PROCESS_T>
    AbstractConnectionPool<PROCESS_T>& 
        AbstractConnectionPool<PROCESS_T>::event_process(
            const Socket& socket, Event events){
        switch(events){
            case Event::In:
                break;
            case Event::Out:
    ...
    template<typename PROCESS_T>
    void AbstractConnectionPool<PROCESS_T>::polling_connections(){
    ...
    template<typename PROCESS_T>
    void AbstractConnectionPool<PROCESS_T>::stop(bool wait_finish, uint16_t timeout_sec){
        if(mp_connections)
