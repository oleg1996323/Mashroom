...
class Array{
    ...
    static std::expected<Array,osterlib::ContextedError> parse(const std::string& input){
        Array processed_tokens;
        auto parse_result = parse_json_from_buffer(input);
        if(!parse_result.has_value() || !parse_result->is_array()){
            osterlib::ContextedError error(mashroom::errc::deserialization_error,"not array");
            error.with_field("file")
