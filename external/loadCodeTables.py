...
def generate_header(path, output_dir):
    ...
    switcher = "const ParmTable* parameter_table(unsigned char center, unsigned char table_version, unsigned char param_num){\n"\
        "    switch(center){\n"
    assertions=""
    source_code = "#include \"generated/code_tables/eccodes_tables.h\"\n"
    #parm_tables:str="constexpr const ParmTable* parm_tables[][]={\n"
    """Генерирует .h-файл с таблицами"""
    header_code = f"""// AUTOGENERATED FILE. DO NOT EDIT.
#pragma once
...
"""
    for file in Path(path).iterdir():
        if file.suffix == '.table' and file.name.startswith('2.') and file.name != '2.table':
            ...
            # else :continue
            header_code += f"extern const ParmTable ParmTable_{table_name.replace('.','_').upper()}[256];\n"
            source_code += f"const ParmTable ParmTable_{table_name.replace('.','_').upper()}[256] = {{\n"
            if len(name_parts) == 2:
            ...
            with open(file, 'r') as f:
                ...
                for line in f:
                    ...
                    if(re.search("missin",comment,re.IGNORECASE)):
                        name = "Missing"
                    elif(name.isdigit()):
                        name = comment.split(' ',1)[0]
                        comment_parts = comment.split(' ',1)
                        if(len(comment_parts)>1):
                            comment = comment_parts[1]
                    while int(num)>count and count<256:
                        source_code += f'    {{"Unknown", "Unknown"}}, //{count}\n'
                        count+=1
                    source_code += fr'    {{"{name}", "{comment}"}}, //{count}'+'\n'
                    count+=1
                while count<256:
                    source_code += f'    {{"Unknown", "Unknown"}}, //{count}\n'
                    count+=1
    ...
    header_code+="extern "+switcher.split("{",1)[0]+";"
    source_code+=assertions
    ...
    # print(parm_tables)
    # code+=parm_tables
    header = f"{output_dir}/include/generated/code_tables/eccodes_tables.h"
    source = f"{output_dir}/src/generated/code_tables/eccodes_tables.cpp"
