...
def generate_header(path, output_dir):
    center_dict = {}
    switcher = "const ParmTable* parameter_table(unsigned char center, unsigned char subcenter, unsigned char param_num){\n"\
        "    switch(center){\n"
    assertions=""
...
"""
    ...
    for center,subcenter_data in center_dict.items():
        switcher+=f"    case {center}:\n"\
        "    switch(subcenter){\n"
        for subcenter,str in subcenter_data.items():
            switcher+=f"        case {subcenter}:\n"\
            f"          return {str};\n"\
            "           break;\n"
        switcher+="        default:\n"\
        "        return NULL;\n"\
    ...
    with open(f"{output_dir}/include/eccodes_tables.h", 'w') as f:
        f.write(code)

    with open(f"{output_dir}/src/eccodes_tables.cpp","w") as f:
        f.write(f"#include \"generated/code_tables/include/eccodes_tables.h\"\n"+
                switcher)
