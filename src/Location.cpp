...
template<>
Location<false> boost::lexical_cast(const std::string& str){
    ...
    if(type==path::TYPE::HOST){
        ...
        if (std::regex_search(str.begin(),str.end(), match, regex_val)) {
                ...
                port<=static_cast<uint64_t>(std::numeric_limits<uint16_t>::max()))
            {
                ...
                if(addr.valid())
                    return Location<false>::host(host,port,std::chrono::system_clock::now());
        ...
        if (std::regex_match(str.begin(),str.end(), match, regex_val)) {
            ...
            if(type==path::TYPE::FILE)
                return Location<false>::file(
                    path,std::chrono::system_clock::now());
            else return Location<false>::directory(
                    path,std::chrono::system_clock::now());
        }
...
template <>
bool CLI::detail::lexical_cast<
    Location<false>>(
        const std::string& input, Location<false>& output)
{
    output = boost::lexical_cast<Location<false>>(input);
    return true;
}