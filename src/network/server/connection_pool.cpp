...
ConnectionPool::iterator ConnectionPool::add_connection(Socket connection_socket,size_t buffer_sz){
...
network::connection::ConnectionPool::ConnectionPool(const Server& server):server_(server){
...
void network::connection::ConnectionPool::shutdown_all(){
    std::for_each(data_.begin(),data_.end(),[](const connection::Process<Server>& conn){
        return conn.shut();
    });
}
void network::connection::ConnectionPool::shut_not_processing(){
    for(auto& conn:data_)
        if(!conn.busy())
            conn.shut();
        else conn.shut_at_done();
}
void network::connection::ConnectionPool::close_all_at_not_busy(){
    std::for_each(data_.begin(),data_.end(),[](const connection::Process<Server>& conn){
        if(!conn.busy())
            return conn.close();
...
void network::connection::ConnectionPool::close_all(){
    std::for_each(data_.begin(),data_.end(),[](const connection::Process<Server>& conn){
        return conn.close();
    });
}
