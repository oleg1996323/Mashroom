...
namespace network{
    void __MessageBase__::assign_buffer(std::vector<char>&& buf) noexcept{
        if(&buf!=&__buffer__())
            __buffer__().swap(buf);
    }
    void __MessageBase__::assign_buffer(const std::vector<char>& buf) const{
        if(&buf!=&__buffer__())
            __buffer__() = buf;
    }
    //add assignment buffer
    std::vector<char>& __MessageBase__::__buffer__() const{
        return CashedData::get(this);
    }
    namespace server{
        template<TYPE_MESSAGE MSG>
        void __Message__<MSG>::__serialize_base_msg__(const __Message__<MSG>& msg){
            auto& buf = msg.__buffer__();
        ...
        {
            if(!msg.__buffer__().empty())
                msg.__buffer__().clear();
