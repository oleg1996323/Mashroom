    ...
    if(tokens.empty())
        ...
        for(std::string_view token:tokens){
            if(token.empty())
                ...
                switch (translate_extract_fmt(token))
                {
                case ExtractFormatArgs::ARCHIVED:{
...
void extract_parse(std::vector<std::string_view> input){
    ...
    for(size_t i=0;i<input.size();++i){
        switch (translate_from_txt(input[i++]))
        {
            ...
            case Command::EXTRACT_FORMAT:{
                extract_out_fmt = get_extract_format(input[i]);
                break;
            }
            case Command::EXTRACTION_DIV:{
