...
ErrorCode host_notifier(const std::string& input) noexcept{   
    if(network::is_correct_address(input)){
        ServerConfigOptions::config()->settings_.host_=input;
        return ErrorCode::NONE;
    }
...
ErrorCode port_notifier(int input) noexcept{
    ...
    ServerConfigOptions::config()->settings_.port_=static_cast<Port>(input);
    return ErrorCode::NONE;
}
...
ErrorCode protocol_notifier(const std::string& input) noexcept{
    ...
    else ServerConfigOptions::config()->settings_.protocol_ = static_cast<network::Protocol>(proto->p_proto);
    return ErrorCode::NONE;
}
