...
namespace parse{
    Index::Index(CLI::App* cli):app_(cli){
        auto jobs_ = app_->add_option("-j,--jobs",
            jobs_val_,
        ...
        add_option("-o,--output-dir",output_dir_val_,
        "Output directory where index files of format specified by 'index-format' will be written.\
        If the path does not exist and can be created, it will be created.\
        Else an error will be occured and the indexing process will not begin.")->
        ...
        //     ))->capture_default_str();
        auto ref_ = app_->add_flag("-r,--reference-only",
            "The date and time (in UTC) up to which\
            the data is extracted.\
            Shall be not less than value of \"dtfrom\"")->
            default_val(true)->
            capture_default_str();
        auto web_file_max_size_ = app_->add_option("--web-file-max-size",info_val_,
    ...
    void Index::execute(){
        ::Index index;
        if(auto jobs_ = app_->get_option("--jobs");jobs_->count())
            index.set_using_processor_cores(jobs_val_);
        if(auto output_dir_= app_->get_option("--output-dir");
                output_dir_->count())
            index.set_dest_dir(output_dir_->as<std::string>());
        if(auto input_paths_= app_->get_option("--input-paths");
                input_paths_->count())
        {
        ...
        if(auto ref_ = app_->get_option("--reference-only");
                ref_->count())
            index.set_host_ref_only();
        if(auto web_file_max_size_ = app_->get_option("--web-file-max-size");
                web_file_max_size_->count() &&
                index.host_ref_only())
