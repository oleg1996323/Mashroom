    ...
    for(const auto& [filename,file_data]:sublimed_grib_data.data()){
        for(const auto& [cmn,sublimed_data_seq]:file_data){
            for(const auto& sublimed_data:sublimed_data_seq){
                utc_tp beg_period = t_off_.get_null_aligned_tp(sublimed_data.sequence_time_.interval_.from_,props_.from_date_);
                utc_tp end_period;
                decltype(sublimed_data.sequence_time_.discret_) discretion =    duration_cast<std::chrono::seconds>(sublimed_data.sequence_time_.discret_)>utc_diff(0)?
                                                                duration_cast<std::chrono::seconds>(sublimed_data.sequence_time_.discret_):std::chrono::seconds(1);
                end_period = t_off_.get_next_tp(sublimed_data.sequence_time_.interval_.from_)-discretion;
                if(sublimed_data.sequence_time_.interval_.from_<=beg_period){
                    if(sublimed_data.sequence_time_.interval_.to_<end_period)
                        accessible_data<<std::vformat(this->time_result_format,std::make_format_args(sublimed_data.sequence_time_.interval_.from_))<<"-"<<
                        std::vformat(this->time_result_format,std::make_format_args(sublimed_data.sequence_time_.interval_.from_))<<std::endl;
                    else missing_log<<sublimed_data.sequence_time_.interval_.to_<<"-"<<end_period<<std::endl;
                }
                else {
                    missing_log<<std::vformat(this->time_result_format,std::make_format_args(beg_period))<<"-"<<
                        std::vformat(this->time_result_format,std::make_format_args(sublimed_data.sequence_time_.interval_.from_))<<std::endl;
                    if(sublimed_data.sequence_time_.interval_.to_<end_period)
                        missing_log<<std::vformat(this->time_result_format,std::make_format_args(sublimed_data.sequence_time_.interval_.to_))<<"-"<<
