...
std::expected<std::vector<data::FileMsg<Data_t::TIME_SERIES,Data_f::GRIB_v1>>,std::exception> from_json(const boost::json::value& val){
    std::vector<data::FileMsg<Data_t::TIME_SERIES,Data_f::GRIB_v1>> result;
    if(auto arr = val.if_array();arr){
        for(auto& msg:*arr)
            if(std::expected<data::FileMsg<Data_t::TIME_SERIES,Data_f::GRIB_v1>, std::exception> res_msg = from_json<data::FileMsg<Data_t::TIME_SERIES,Data_f::GRIB_v1>>(msg);res_msg.has_value())
